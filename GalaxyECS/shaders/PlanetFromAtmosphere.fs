...
void main()
{
    ...
    //if(brightness > 1.0)
        //BrightColour = vec4(result, 1.0) * 1.0;
    //else
        //BrightColour = vec4(0.0, 0.0, 0.0, 1.0);

    vec4 result = frontColor + c * secondaryColor;
    //vec4 result = (frontColor * secondaryColor) + c;
    result.a = 1.0;
    //vec4 result = vec4((ambient + diffuseSh) * c.xyz, 1.0);


    FragColour = result;

    float brightness = dot(result.xyz, vec3(0.2126, 0.7152, 0.0722));
    if (brightness > 1.0) BrightColour = result;
    else BrightColour = vec4(0.0, 0.0, 0.0, 1.0);

    //FragColour = vec4(result, 1.0);
    //gl_FragColor = FragColour;//vec4(result, 1.0);
    //gl_FragColor = c * clamp(dot(normalize(lightDir), normalize(normal0)), 0, 1);
