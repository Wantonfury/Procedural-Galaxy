...
void main()
{
    ...
    //vec3 fPosition = vec3(coord.x * coord.y - 0.05, coord);
    vec3 fPosition = vec3(coord.x * coord.y, coord);

    float t = (dt * 2.0) - length(coord);
    ...
    float dist = (length(position + offsetVec)) * 10.0;
    float brightness = (1.0 / (dist * dist) - 0.1) * 0.7;
    if (brightness < 0.001) brightness = 0.0;

    FragColour = vec4(sunColour * 2.0, brightness);
    BrightColour = FragColour * 0.2;
}